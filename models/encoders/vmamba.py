...
class VSSM(nn.Module):
    def __init__(
        ...
        # 用Conv2d将输入图像分块并升维，类似ViT/Swin的PatchEmbedding。
        # 使用Permute调整维度顺序为 (B, H, W, C)，方便后续处理，可选归一化。

        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]  
        # stochastic depth decay rule
        # dpr是一个列表，包含了每个块的drop path概率，从0到drop_path_rate线性变化。
        ...
        for i_layer in range(self.num_layers):
            ...
            # VSSM 主干网络构建每个 stage（层组）的核心部分
            '''
            VSSM 主干网络构建每个 stage（层组）的核心部分。
            它的作用是逐层堆叠多个“block序列+下采样”结构，实现分层特征提取和空间分辨率递减，
            类似于 Transformer/Swin Transformer 的 stage 设计。
            '''
...
class Backbone_VSSM(VSSM):
    ...
    def forward(self, x):
        ...
        return outs
    # backbone的输入是BCHW格式的图像张量，
    # 输出是一个包含多个尺度特征图的列表，不同分辨率特征
